...
{
public:
	CWorker(const std::string name, std::string post, const std::shared_ptr<CCompany> &company,
		unsigned age, unsigned height, unsigned weight, bool isMale);
	
	std::string GetPost() const;
	std::shared_ptr<CCompany> GetCompany() const;
...
private:
	std::string m_post;
	std::weak_ptr<CCompany> m_company;
};
